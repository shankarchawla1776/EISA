...
```
This method combats common issues like memory staleness. This approach attributed an integer weight to every episode in the episodic memory bank. This is where the most chronologcially dated element has a weight 1 ($\omega_{1}$) and the newest has weight n (len(self.mem) or $\omega_{n}$). This weight is then considered during retrival. As the weight of an episode approaches 1, the probability of its retrival approaches 0. This probability will fluxuate 